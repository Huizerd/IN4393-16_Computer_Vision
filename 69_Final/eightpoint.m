...
function [F_ransac_denorm, inliers_best] = eightpoint(x1, y1, x2, y2, best_matches, threshold)
...
%% Normalization
% For  first image
xcoords = x1(:,best_matches(2,row));
ycoords = y1(:,best_matches(2,row));
...
% For second image
xcoords = x2(:,best_matches(3,row));
ycoords = y2(:,best_matches(3,row));
